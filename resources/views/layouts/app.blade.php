...
<head>
    ...
    <style>
        /* Jalan pulang ke aplikasi, hanya saat halaman dibuka dari PWA terpasang */
        .pwa-return { display: none; }

        @media all and (display-mode: standalone) {
            .pwa-return { display: inline-flex; }
        }

        html.is-standalone .pwa-return { display: inline-flex; }
    </style>

    @unless (request()->routeIs('rpp.show', 'rpp.print'))
        <script>
            // Aplikasi yang sudah dipasang tetap di dalam PWA, bukan tampilan desktop.
            // Halaman isi modul dikecualikan karena diakses dari menu Detail.
            if (window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true) {
...
<body class="font-sans antialiased"
    ...
    <div x-data="{ open: false }" @toggle-sidebar.window="open = !open" x-show="open" x-cloak class="lg:hidden">
        ...
        <aside
            class="fixed inset-y-0 left-0 z-50 w-64 bg-[hsl(var(--background))] border-r border-[hsl(var(--border))]"
            ...
            <div class="flex flex-col h-full">
                <!-- Logo & Close Button -->
                <div class="flex items-center justify-between gap-3 px-6 py-5 border-b border-[hsl(var(--border))]">
                    <div class="flex items-center gap-3">
                        <img src="{{ asset('logo.png') }}" alt="Logo" class="w-8 h-8 object-contain">
                        <span class="font-semibold text-[hsl(var(--foreground))]">RPP Generator</span>
                    </div>
                    <button type="button" @click="open = false"
                        class="p-2 rounded-lg hover:bg-[hsl(var(--accent))]">
