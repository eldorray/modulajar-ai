<x-app-layout>
    ...
    <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <x-ui.card>
                    <div class="text-center">
                        <p class="text-3xl font-bold text-[hsl(var(--foreground))]">{{ $adminStats['total_users'] }}</p>
                        <p class="text-sm text-[hsl(var(--muted-foreground))]">Total Users</p>
                    </div>
                </x-ui.card>
                <x-ui.card>
                    <div class="text-center">
                        <p class="text-3xl font-bold text-[hsl(var(--foreground))]">{{ $adminStats['total_rpps'] }}</p>
                        <p class="text-sm text-[hsl(var(--muted-foreground))]">Total RPP</p>
                    </div>
                </x-ui.card>
                <x-ui.card>
                    <div class="text-center">
                        <p class="text-3xl font-bold text-[hsl(var(--foreground))]">{{ number_format($adminStats['deepseek']['total_tokens'] + $adminStats['gemini']['total_tokens']) }}</p>
                ...
                <x-ui.card class="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
                    <div class="text-center">
                        <p class="text-2xl font-bold text-green-700">Rp {{ number_format($adminStats['total_cost_idr'], 0, ',', '.') }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <x-ui.card class="border-blue-200">
                    ...
                    <div class="grid grid-cols-3 gap-4 text-center">
                        <div>
                            <p class="text-xl font-bold text-blue-700">{{ number_format($adminStats['deepseek']['total_tokens']) }}</p>
                            <p class="text-xs text-[hsl(var(--muted-foreground))]">Total Tokens</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">{{ number_format($adminStats['deepseek']['input_tokens']) }} in</p>
                            <p class="text-sm text-gray-600">{{ number_format($adminStats['deepseek']['output_tokens']) }} out</p>
                        </div>
                        <div>
                            <p class="text-lg font-bold text-green-600">Rp {{ number_format($adminStats['deepseek']['cost']['total_cost_idr'], 0, ',', '.') }}</p>
                ...
                <x-ui.card class="border-purple-200">
                    ...
                    <div class="grid grid-cols-3 gap-4 text-center">
                        <div>
                            <p class="text-xl font-bold text-purple-700">{{ number_format($adminStats['gemini']['total_tokens']) }}</p>
                            <p class="text-xs text-[hsl(var(--muted-foreground))]">Total Tokens</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">{{ number_format($adminStats['gemini']['input_tokens']) }} in</p>
                            <p class="text-sm text-gray-600">{{ number_format($adminStats['gemini']['output_tokens']) }} out</p>
                        </div>
                        <div>
                            <p class="text-lg font-bold text-green-600">Rp {{ number_format($adminStats['gemini']['cost']['total_cost_idr'], 0, ',', '.') }}</p>
                            <p class="text-xs text-[hsl(var(--muted-foreground))]">Biaya</p>
                        </div>
                    </div>
                </x-ui.card>
        ...
        <!-- User Token Usage -->
        @if($userTokens && $userTokens->total_tokens > 0)
        <x-ui.card>
            <x-slot name="header">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="text-center bg-green-50 rounded-lg py-2">
                    ...
                    <p class="text-lg font-bold text-green-700">Rp {{ number_format($userCost['total_cost_idr'], 0, ',', '.') }}</p>
                </div>
            </div>
        </x-ui.card>
        @endif
        ...
        <div>
            ...
            <x-ui.card>
                <div class="text-center py-8">
                    ...
                    <p class="text-[hsl(var(--muted-foreground))] mb-4">Mulai buat RPP pertama Anda dengan bantuan AI.</p>
