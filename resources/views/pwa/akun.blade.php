<x-pwa-layout title="Akun" active="akun">
    ...
    <section class="pwa-card pop-in overflow-hidden" style="--d: 130ms">
        @php
            $links = [
                ['Ubah profil', route('profile.edit'), 'M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM4.5 20a7.5 7.5 0 0 1 15 0', false],
                ['Pengaturan sekolah', route('settings.index'), 'M4 7h16M4 12h16M4 17h10', false],
                ['Dashboard desktop', route('dashboard'), 'M4 5.5h16v9.5H4zM9 19h6', true],
            ];
            if ($user->isAdmin()) {
                $links[] = ['Pengaturan AI', route('admin.ai.edit'), 'M13 10V3L4 14h7v7l9-11h-7z', false];
            }
        @endphp
        @foreach ($links as $i => [$label, $url, $icon, $hideStandalone])
            <a href="{{ $url }}" class="press flex items-center gap-3 px-4 py-3.5 {{ $i > 0 ? 'border-t' : '' }}"
