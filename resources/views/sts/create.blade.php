    ...
    <div class="max-w-3xl mx-auto">
        <x-ui.card>
            <x-slot name="header">
                ...
                <p class="text-sm text-[hsl(var(--muted-foreground))] mt-1">Isi data lengkap untuk menghasilkan Soal
                    Sumatif Tengah Semester dengan AI.</p>
            </x-slot>
            ...
            <form id="sts-form" action="{{ route('sts.store') }}" method="POST" class="space-y-6" x-data="{ loading: false }">
                ...
                <div class="space-y-4 pb-4 border-b border-[hsl(var(--border))]">
                    <h3 class="text-sm font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wide">
                        Informasi Umum</h3>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-ui.input name="mata_pelajaran" label="Mata Pelajaran"
                            placeholder="Contoh: Matematika, Bahasa Indonesia, IPA" :value="old('mata_pelajaran')" :error="$errors->first('mata_pelajaran')"
                            required />

                        <x-ui.select name="fase" label="Fase/Jenjang" :options="[
                            ...
                            placeholder="Pilih Fase/Jenjang" :value="old('fase')" :error="$errors->first('fase')" required />
                    </div>

                    <x-ui.input name="kelas" label="Kelas" placeholder="Contoh: 7, 8, 10, 11" :value="old('kelas')"
                        :error="$errors->first('kelas')" required />
                </div>

                <!-- Materi -->
                <div class="space-y-4 pb-4 border-b border-[hsl(var(--border))]">
                    <h3 class="text-sm font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wide">Materi
                        & Tujuan</h3>

                    <x-ui.textarea name="topik" label="Topik/Materi"
                        placeholder="Contoh: Teks Deskripsi dan Puisi Rakyat, Operasi Bilangan Bulat" rows="2"
                        :error="$errors->first('topik')" required>{{ old('topik') }}</x-ui.textarea>

                    <x-ui.textarea name="tujuan_pembelajaran" label="Tujuan Pembelajaran (opsional)"
                        placeholder="Salin TP dari modul ajar jika ada, atau biarkan AI merumuskan berdasarkan topik"
                        rows="3" :error="$errors->first('tujuan_pembelajaran')">{{ old('tujuan_pembelajaran') }}</x-ui.textarea>

                    <x-ui.textarea name="materi" label="Materi Tambahan (opsional)"
                        placeholder="Tempelkan materi yang akan dijadikan dasar pembuatan soal. Semakin detail, semakin baik hasilnya."
                        rows="4" :error="$errors->first('materi')">{{ old('materi') }}</x-ui.textarea>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <x-ui.input type="number" name="jumlah_pg" label="Pilihan Ganda" :value="old('jumlah_pg', 5)"
                            :error="$errors->first('jumlah_pg')" min="0" max="30" required />

                        <x-ui.input type="number" name="jumlah_pg_kompleks" label="PG Kompleks" :value="old('jumlah_pg_kompleks', 2)"
                            :error="$errors->first('jumlah_pg_kompleks')" min="0" max="10" required />

                        <x-ui.input type="number" name="jumlah_menjodohkan" label="Menjodohkan" :value="old('jumlah_menjodohkan', 3)"
                            :error="$errors->first('jumlah_menjodohkan')" min="0" max="10" required />

                        <x-ui.input type="number" name="jumlah_uraian" label="Uraian/Esai" :value="old('jumlah_uraian', 2)"
                            :error="$errors->first('jumlah_uraian')" min="0" max="10" required />
                    </div>
        ...
        <!-- Loading Overlay -->
        <div x-data="progressLoader()" x-show="show" x-cloak
            ...
            <div class="bg-white rounded-3xl p-8 shadow-2xl max-w-md mx-4 text-center w-full">
                ...
                <template x-if="!isComplete && !hasError">
                    <div>
                        <div class="mb-6">
                            <img src="{{ asset('refrensi/loading.gif') }}" alt="Loading Animation"
                ...
                <div class="mb-4" x-show="!hasError">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-sm font-bold" :class="isComplete ? 'text-green-600' : 'text-blue-600'"
                ...
                <div class="space-y-2 text-left bg-gray-50 rounded-xl p-4 mt-4" x-show="!hasError">
                    <div class="flex items-center gap-3" :class="progress >= 20 ? 'text-green-600' : 'text-gray-400'">
                        ...
                        <template x-if="progress < 20">
                            <div class="w-4 h-4 border-2 border-current rounded-full animate-pulse flex-shrink-0">
                            </div>
                        </template>
                        <span class="text-sm" :class="progress >= 20 ? 'font-medium' : ''">📋 Menganalisis
                            materi</span>
                    </div>
                    ...
                    <div class="flex items-center gap-3" :class="progress >= 45 ? 'text-green-600' : 'text-gray-400'">
                        ...
                        <span class="text-sm" :class="progress >= 45 ? 'font-medium' : ''">📝 Menyusun soal &
                            kisi-kisi</span>
                    </div>
                    ...
                    <div class="flex items-center gap-3" :class="progress >= 70 ? 'text-green-600' : 'text-gray-400'">
                        ...
                        <span class="text-sm" :class="progress >= 70 ? 'font-medium' : ''">✅ Membuat kunci
                            jawaban</span>
                    </div>

                    <div class="flex items-center gap-3"
                        :class="progress >= 100 ? 'text-green-600' : 'text-blue-500'">
                        ...
                        <span class="text-sm"
                            :class="progress >= 100 ? 'font-medium text-green-600' : 'font-semibold'">🚀 Menyusun
                            rubrik penilaian</span>
                    </div>
                </div>

                <p class="text-xs text-gray-400 mt-4" x-show="!isComplete && !hasError">⏱️ Estimasi waktu: 60-120
                    detik</p>
            </div>
        </div>
            ...
            function progressLoader() {
                return {
                    ...
                    startProgress() {
                        const steps = [{
                                at: 5,
                                text: 'Mengirim data ke AI...'
                            },
                            {
                                at: 20,
                                text: 'Menganalisis materi...'
                            },
                            {
                                at: 45,
                                text: 'Menyusun soal & kisi-kisi...'
                            },
                            {
                                at: 70,
                                text: 'Membuat kunci jawaban...'
                            },
                            {
                                at: 85,
                                text: 'Menyusun rubrik penilaian...'
                            },
                            {
                                at: 95,
                                text: 'Hampir selesai...'
        ...
        <div class="mt-6">
            <x-ui.alert type="info">
                <strong>Tips:</strong> Semakin detail topik dan materi yang Anda masukkan, semakin berkualitas soal yang
                dihasilkan AI.
            </x-ui.alert>
        </div>
    </div>
